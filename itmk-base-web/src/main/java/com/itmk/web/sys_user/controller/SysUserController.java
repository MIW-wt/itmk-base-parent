...
//        QueryWrapper<SysUser> query = new QueryWrapper<>();
//        query.lambda().eq(SysUser::getIsAdmin, userId);
//        List<SysUser> list1 = sysUserService.list(query);
//        System.out.println(list1.size());
//        if (list1.size() <= 1) {
//            return ResultUtils.error("唯一的管理原账户，不能删除！");
//        }
        boolean remove = sysUserService.deleteByUserId(userId);
        if (remove) {
